...
| 구성 요소 | 종류 | 용도 |
| --- | --- | --- |
| [`OdbContext`](Packages/src/Runtime/OdbContext.cs) | 추상 클래스 | 모델을 구성하고 데이터 집합과 스냅샷 입출력을 관리한다. |
...
| `OdbXmlSnapshotImporter` / `OdbXmlSnapshotExporter` | 스냅샷 | XML 형식으로 데이터베이스를 가져오고 내보낸다. |
| `OdbJsonSnapshotImporter` / `OdbJsonSnapshotExporter` | 스냅샷 | JSON 형식으로 데이터베이스를 가져오고 내보낸다. |
...
- `InitializeAsync`는 기존 데이터를 비운 뒤 가져오며 실패 시 이전 상태로 되돌리지 않는다.
